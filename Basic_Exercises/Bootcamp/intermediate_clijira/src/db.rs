...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        let mut parse = self.database.read_db()?;

        let last_item_id = parse.last_item_id;
        let new_id = last_item_id + 1;
        parse.last_item_id = new_id;
        parse.epics.insert(new_id, epic);
        self.database.write_db(&parse)?;
        Ok(new_id)
    }
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let new_id = last_item_id + 1;
        parse.last_item_id = new_id;
        parse.stories.insert(new_id,story);
        parse.epics.get_mut(&epic_id)
            .ok_or_else(|| anyhow::anyhow!("Epic with id {} not found", epic_id))?
            .stories.push(new_id);
        self.database.write_db(&parse)?;
        Ok(new_id)
    }
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        if !parse.epics.contains_key(&epic_id) {
            return Err(anyhow::anyhow!("Epic with id {} not found", epic_id));
        }
        
        for story_id in &parse.epics.get(&epic_id)
            .ok_or_else(|| anyhow::anyhow!("Epic with id {} not found", epic_id))?
            .stories {
                parse.stories.remove(story_id);
        }

        parse.epics.remove(&epic_id);
        self.database.write_db(&parse)?;
        Ok(())
    }
    ...
    pub fn delete_story(&self,epic_id: u32, story_id: u32) -> Result<()> {
        ...
        if !parse.stories.contains_key(&story_id) {
            return Err(anyhow::anyhow!("Story with id {} not found", story_id));
        }

        let epic = parse.epics.get_mut(&epic_id)
            .ok_or_else(|| anyhow::anyhow!("Epic with id {} not found", epic_id))?;
        ...
        if let Some(index) = story_index {
            ...
            return Err(anyhow::anyhow!("Story with id {} not found in epic {}", story_id, epic_id));
        }
        // Remove the story from the stories map
        parse.stories.remove(&story_id);
        self.database.write_db(&parse)?;
        Ok(())
    }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        let story = parse.stories.get_mut(&story_id)
            .ok_or_else(|| anyhow::anyhow!("Story with id {} not found", story_id))?;
        story.status = status;
        self.database.write_db(&parse)?;
        Ok(())
    }
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        Ok(db_state) // deserialize json content to DBState
    }
